...
static const char *save_xpm[] = {"16 16 9 1",        " c None",          ". c #000000",      "+ c #0000FF",
                                 "@ c #000080",      "# c #A0A0A0",      "$ c #0000C0",      "% c #FFFFFF",
                                 "& c #C0C0FF",      "* c #DCDCDC",      "................", ".+@##########@+.",
                                 ".$@%%%%%%%%%%@$.", ".+@%%%%%%%%%%@+.", ".+@%########%@+.", ".+@%%%%%%%%%%@+.",
                                 ".+@%########%@+.", ".+@&%%%%%%%%&@+.", ".++@@@@@@@@@@++.", ".++++++++++++++.",
                                 ".++$......@@$++.", ".++.******.+@++.", ".++.*@+***.+@++.", ".++.*@+***.+@++.",
...
void Toolbar::cb_save(Fl_Widget *w, void *data) {
  fl_alert("Save Button %ld pressed!", (long)data);
}
void Toolbar::cb_fonts(Fl_Widget *w, void *data) {
  fl_alert("Font number %ld selected!", (long)data);
}
...
Toolbar::Toolbar(int Xpos, int Ypos, int Width, int Height)
  ...
  Width = Height;
  int i;
  save1 = new Fl_Button(Xpos, Ypos, Width, Height);
  Xpos += Width + 5;
  save2 = new Fl_Button(Xpos, Ypos, Width, Height);
  Xpos += Width + 5;
  font_group = new Fl_Choice(Xpos, Ypos, 110, Height);
  Xpos += 111;
...
int main() {
  ...
  Toolbar tool(0, TOOLBAR_Y, win.w(), 25);
